...
# 계열사 이름은 회사 이름을 따라간다.
#
# ★ 회사 이름을 바꿨는데 이 칸들만 옛 이름으로 남은 적이 있다 —
#   배치도에 「누리랩(주)」가 그대로 떠 있었다. 글자로 박아 두면
#   company.py --apply 도 못 잡는다(그 목록에 없는 낱말이므로).
#   여기서 가져다 쓰면 이름을 한 번 바꿀 때 같이 따라온다.
try:
    from company import COMPANY
except ImportError:                      # 컨테이너 안에서 부를 때
    from tools.company import COMPANY
...
CELLS_3F = [
    ...
    (12, 15, 1, 1, "LABEL", f"{COMPANY}랩(주)",    None),
    ...
    (15, 15, 1, 1, "LABEL", f"{COMPANY}랩",        None),
...
CELLS_4F = [
    ...
    (8, 18, 1, 2, "ROOM",  f"{COMPANY}문화재단",   None),
