# 사내 AI 챗 포털
> 체험판 **https://portal.trendkit.app/** (demo / demo1234)
> **왜 이렇게 만들었는지**는 [케이스 스터디](https://www.9seebird.tech/portal-case-study.html)에 있습니다.
> 이 문서는 무엇이 들어 있고, 어떻게 띄우고, 체험판에서 무엇이 막혀 있는지를 다룹니다.

직원이 **주소 하나**로 들어와서, **한국어로 물어보면** 사내 앱이 대신 일해 주는 포털입니다.
로그인·권한·이력·AI 비용 관리가 한곳에 있고, 그 뒤에 업무 앱 아홉 개가 붙어 있습니다.
...
| **IT 안내** | 매뉴얼 · 운영 체크리스트 · 비용 · 프롬프트 생성기 · 사내앱 규칙 · AI 참고자료 · 자가진단 | 정적 페이지 7장 · 챗이 그 안을 검색 |
| **온라인 교육 이수** | 지정 영상 시청 진도 → 자동 이수 · 이수율 | **스크롤바로 건너뛰면 이수 안 됨** |
...
같은 구조로 붙어 있는 앱 하나를 예로 보면 이렇습니다 — 직원은 자기 진도만,
담당자는 전사 이수율을 봅니다. 같은 앱, 같은 데이터, 권한만 다릅니다.

![온라인 교육 · 직원 화면](docs/07-edu.png)

![온라인 교육 · 담당자 화면](docs/08-edu-admin.png)
...
<details>
<summary><b>진도는 '어디까지 갔나'가 아니라 '어디를 지났나'로 센다</b></summary>

전 직원 교육을 무료 강의로 돌리면 콘텐츠는 아쉽지 않습니다. 잃는 것은
**수강 기록** 하나입니다. 그래서 그 기록만 회사가 갖게 만들었습니다.

처음에는 최대 재생 위치를 저장했습니다. 그러면 **스크롤바를 맨 뒤로 한 번
끄는 것으로 이수가 끝납니다.** 그래서 영상을 10초짜리 칸으로 쪼개고,
화면이 5초마다 보내오는 위치로 **지나간 칸만** 채웁니다.

- 직전 위치와 지금 위치의 차이가 30초 안이면 그 사이를 이어 본 것으로 인정
  → 1.5배·2배속도 정상 기록
- 차이가 크면 건너뛴 것이므로 **지금 칸 하나만** 채움
  → 맨 뒤로 끌어도 중간이 비어 이수되지 않음

영상은 이 앱이 내보내지 않습니다. 유튜브가 재생하고 앱은 위치만 받아 적어서,
몇 명이 붙어도 서버 부하가 사실상 없습니다. **자체 서빙을 택하지 않은 이유**는
uvicorn 워커가 스트림을 붙들고 있는 동안 그 앱의 다른 요청이 밀리기 때문입니다.

이수 기준은 90%입니다. 100%로 두면 엔딩 카드까지 보는 사람이 드물어서
**다 본 사람이 미이수로 남습니다.**
</details>

<details>
<summary><b>영상을 바꾸면 그 차시 진도를 버린다</b></summary>

담당자가 차시의 영상 주소를 다른 것으로 고칠 수 있습니다. 그때 진도를 그대로
두면 **옛 영상에서 본 15분이 새 영상의 15분으로 둔갑합니다** — 안 본 강의가
이수로 잡히는 셈입니다. 그래서 영상이 바뀌면 그 차시의 진도만 초기화하고,
제목·안내문·기준만 고칠 때는 건드리지 않습니다.
</details>

<details>
<summary><b>사번이 아니라 계정 아이디를 열쇠로 썼다</b></summary>

교육 앱은 포털 없이 혼자 돌 수도 있습니다(`AUTH_MODE`). 포털이 아직 전 직원에게
열리지 않은 상태에서 교육을 먼저 돌려야 할 때를 위해서입니다.

이때 무엇을 사람의 열쇠로 쓸지가 갈림길이었습니다. 직원은 사번을 더 잘 알지만,
사번으로 기록을 쌓아 두면 **나중에 포털에 붙일 때 그동안의 이수 기록이
끊깁니다**(포털이 넘겨주는 것은 계정 아이디이므로). 그래서 저장은 아이디로 하고,
로그인할 때만 사번으로도 찾아 주게 했습니다. 두 방식이 같은 모양의 사용자
정보를 만들어 내므로 나머지 코드는 어느 쪽으로 들어왔는지 알지 못합니다.
</details>
...
- `proxy/conf.d/00-demo-guard.conf` — 앱 아홉 개를 전부 본다.
...
```
docker build -t portfolio-tools tools
docker run --rm -v "%CD%:/w" -w /w portfolio-tools python tools/make_demo_data.py --force
```
