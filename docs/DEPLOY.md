...
**공개 주소(80)에서는 누르는 것이 막혀 있습니다.** 방문자와 만든 사람을
화면에서 구분하지 않기로 했기 때문입니다 — 화면에서 구분하려면 「이 계정만은
통과」 같은 예외를 두어야 하고, 그 순간 비밀번호 하나가 안전장치 전부가 됩니다.

대신 **문을 하나 더 냈습니다.** `proxy/conf.d/02-admin.conf` 가 8082 포트에
같은 화면을 한 번 더 띄우는데, 여기에는 체험판 가드가 걸려 있지 않습니다.
안전장치는 비밀번호가 아니라 **네트워크**입니다.

* 8082 는 `127.0.0.1` 로만 열립니다 (`proxy/docker-compose.yml`). 인터넷에서도
  사내망에서도 이 포트에 닿을 수 없습니다.
* 서버의 127.0.0.1 에 닿으려면 SSH 로 들어와야 합니다. 즉 **서버 SSH 키를 가진
  사람만** 이 문을 쓸 수 있습니다 — 이미 서버를 다 만질 수 있는 사람입니다.
* 로그인은 그대로입니다. 포털의 권한 검사는 8082 에서도 똑같이 돕니다.
  없어진 것은 「체험판이라 저장 금지」 한 겹뿐입니다.

라우팅은 `proxy/conf.d/_routes.inc` 한 곳에만 있고 80 과 8082 가 같이 읽습니다.
앱을 새로 붙여도 두 입구가 갈라지지 않습니다.

**쓰는 법**

```bash
# 로컬(회사 PC · 집 PC) — 그냥 열면 됩니다
#   http://127.0.0.1:8082/

# 서버(Azure) — 내 PC에서 터널을 뚫고 같은 주소로 들어갑니다
ssh -L 8082:127.0.0.1:8082 <계정>@<서버주소>
#   → 브라우저에서 http://127.0.0.1:8082/
```

터널은 그 SSH 창이 열려 있는 동안만 삽니다. 창을 닫으면 문도 같이 닫힙니다.

**보는 것은 공개 주소에서도 전부 열려 있습니다.** 이력 · AI 사용량 ·
사용자 명부 · 서비스 목록은 `demo` 계정으로 그냥 보입니다. 서버에서
한 줄로 끝나는 일들은 아래와 같습니다.
...
| 데모 내용을 화면에서 고치기 | 위의 8082 입구로 들어가서 평소처럼 누르면 됩니다 |
